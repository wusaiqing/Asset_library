...
define(function(require, exports, module) {	
    ...
    var courseScoreEnter = {
        // 初始化
        init : function() {
        	// 加载查询条件
        	courseScoreEnter.initQuery();
        	
        	//学年学期或者开课单位变动，加载课程
			$("#semester,#openDepartmentId").change(function(){
			    ...
			    $("#teacherNoAndName").empty().append('<option value="">--请选择--</option>');  // 任课教师
		    	$("#theoreticalTaskId").empty().append('<option value="">--请选择--</option>');  // 教学班号
		    	
		    	courseScoreEnter.showCurrentSelectedInfo();	
			});
			...
			$("#courseId").change(function(){
			    ...
			    if(courseId != dataConstant.EMPTY){ // 课程不为“--请选择--”，则加载任课教师信息
			    	simpleSelect.loadSelect("teacherNoAndName",
			    			url.GET_TEACHER_SELECT_LIST, {
								isAuthority : true,
								semester:semester,
								openDepartmentId:openDepartmentId,
								courseId:courseId								
			...
			$("#teacherNoAndName").change(function(){
			    ...
			    if(teacherNoAndName != dataConstant.EMPTY){ // 任课教师不为“--请选择--”，则加载任教学班号信息
			    	simpleSelect.loadSelect("theoreticalTaskId",
			    			url.GET_THEORETICAL_TASK_SELECT_LIST, {
								isAuthority : true,
								semester:semester,
								openDepartmentId:openDepartmentId,
								courseId:courseId,	
			...
			//教学班变动，加载显示信息
			$("#theoreticalTaskId").change(function(){
				courseScoreEnter.showCurrentSelectedInfo();			    
			});
			...
			$(document).on("blur", "input[myattr='score']", function() {
				courseScoreEnter.calculationTotalScore($(this).attr("index"));
			});
			...
			$('#btnSave').click(function() {
				courseScoreEnter.save(approvalStatus.TemporaryMemory.value); // 审核状态(1-暂存，2-已送审，3-已审核)
			});
			
			// 送审
			$('#btnReview').click(function() {
				courseScoreEnter.review(approvalStatus.Submitted.value); // 审核状态(1-暂存，2-已送审，3-已审核)							
			});		
        }, 
        ...
        showCurrentSelectedInfo:function(){
        	...
        	var txtTeacher=($("#teacherNoAndName").find("option:selected").text()===dataConstant.PLEASE_SELECT)?"":$("#teacherNoAndName").find("option:selected").attr("title");
        	var txtTeachingClassNo=($("#theoreticalTaskId").find("option:selected").text()===dataConstant.PLEASE_SELECT)?"":$("#theoreticalTaskId").find("option:selected").attr("title");
        	
		    $("#spanCourse").text("课程："+txtCourse);			    
		    $("#spanTeacher").text("任课教师："+txtTeacher);			   
			$("#spanTeachingClassNo").text("教学班号："+txtTeachingClassNo);			
			$("#spanScoreSet").text("总评成绩：");			
			
			$("#spanCourse").attr("title","课程："+txtCourse);
			$("#spanTeacher").attr("title","任课教师："+txtTeacher);
			$("#spanTeachingClassNo").attr("title","教学班号："+txtTeachingClassNo);
			$("#spanScoreSet").attr("title","总评成绩：");
        },
		...
		getInitData:function(){
			courseScoreEnter.initShow(); // 查询绑定列表前，初始化列表头部显示
					...
					if (data.code == config.RSP_SUCCESS) {
						if(data.data && data.data.scoreRegimenName!="" ){
							...
							ajaxData.request(url.GET_COURSE_SCORE_ENTRY_LIST, queryParams, function(data) {
								if (data.code == config.RSP_SUCCESS) {
									if(data.data && data.data.length>0 ){
										...
										//绑定特殊情况下拉框
										courseScoreEnter.bindSpecialCaseSelect();
																			
									}else{
										courseScoreEnter.bindNoData();
									}
									courseScoreEnter.calculationWaitEnterCount(); // 计算 教学班共有 X条 未完成录入	
									//判断当前教学班成绩是否已经送审
									if(data.data && data.data.length>0){
										if(courseScoreEnter.haveWaitAudit(data.data)){ // 只要有一个已送审或者已审核，则整个页面不可编辑
							...
							if(data.data && data.data.allowModify === 0){
								...
								popup.askPop("请先设置成绩构成",function(){
									//打开成绩构成页面
									//调用后台，判断是否可以进行修改
		...
		initShow:function(){
			...
			$("#set").show(); // 让构成链接先显示
			courseScoreEnter.calculationWaitEnterCount(); // 计算 教学班共有 0条 未完成录入	
		},
				...
				if (data.code == config.RSP_SUCCESS) {
					if(data.data && data.data.length>0){
						...
						if(courseScoreSet.scoreRegimenName!==dataConstant.HANDROD_CREDIT_NAME){ // 等级制
							//绑定等级制情况下的总评成绩下拉框
							courseScoreEnter.bindScoreRegimenDetailSelect();
						}else{ // 百分制
        ...
        getSaveData:function(){        	
			...
			saveParams["courseScoreSetId"]=courseScoreSet.courseScoreSetId; // 课程/补考成绩录入相关设置id
			saveParams["scoreRegimenId"]=courseScoreSet.scoreRegimenId; // 分制id
        	var list=[]; // 要保存的数据
        	//遍历设置了总评成绩的
			$("#tbodycontent input[name='totalScore']").each(function () {	
					...
					if(courseScoreSet && courseScoreSet.scoreRegimenName && courseScoreSet.scoreRegimenName===dataConstant.HANDROD_CREDIT_NAME){
		...
		save:function(auditStatus){
			var saveParams=courseScoreEnter.getSaveData(); // 获取要保存的数据	
		...
		review:function(auditStatus){
			var count=courseScoreEnter.calculationWaitEnterCount(); // 教学班未完成录入数
			if(count>0){
				...
				popup.askPop("送审后将不能再修改，确定送审吗？",function(){
					courseScoreEnter.save(approvalStatus.Submitted.value); // 审核状态(1-暂存，2-已送审，3-已审核)
				});				
			}	
		},
		...
		calculationTotalScore:function(index){
			...
			courseScoreEnter.validateScore("usualRatio","usualScore",index);
			courseScoreEnter.validateScore("midtermRatio","midtermScore",index);
			courseScoreEnter.validateScore("endtermRatio","endtermScore",index);
			courseScoreEnter.validateScore("skillRatio","skillScore",index);
			...
			if(specialCaseId===""){ //第一个分支，判断总评成绩是否由特殊情况决定
				...
				if(isBFZ){// 第二个分支，判断总评成绩的分制 （百分制or等级制）
					...
					var scoreRegimenDetailId=courseScoreEnter.getScoreRegimenDetailId(score);
    ...
    module.exports = courseScoreEnter;
    window.courseScoreEnter = courseScoreEnter;
});