...
define(function(require, exports, module) {	
    ...
    var courseScoreEnter = {
		...
		indexInit:function(){
			...
			courseScoreEnter.bindIndexList();
			...
			$(document).on("click", "[name='entry']", function() {
				courseScoreEnter.popEntry(this);
			});
			
			// 查看
			$(document).on("click", "[name='view']", function() {
				courseScoreEnter.popView(this);
			});
		},
...
//			
//			console.log(this.courseScoreEnter);
//			this.courseScoreEnter.button({
//                name: '保 存',
//                disabled: true
//            }).button({
//                name: '送 审',
...
//            });
			...
			//$(parent.document.body).find("type='button'").attr("disabled","disabled");
			
			//$(parent.document.body).find(".aui_state_highlight").attr("disabled","disabled");
...
//            });
		...
		save:function(myDialog,iframeObj,auditStatus){
			var saveParams=courseScoreEnter.getSaveData(iframeObj); // 获取要保存的数据	
		...
		review:function(myDialog,iframeObj,auditStatus){
			var count=courseScoreEnter.calculationWaitEnterCount(iframeObj); // 教学班未完成录入数
			if(count>0){
				...
				popup.askPop("送审后将不能再修改，确定送审吗？",function(){
					courseScoreEnter.save(myDialog,iframeObj,approvalStatus.Submitted.value); // 审核状态(1-暂存，2-已送审，3-已审核)
				});				
			}	
		},
		...
		popEntry: function(obj){				
			...
			var myDialog= popup.open('./teacherservice/score/html/courseScoreEnterList.html',
				{
					id : 'scoreEntry',// 唯一标识
					title : '录入课程成绩',// 这是标题
					...
					button:[{name:'保 存',callback:function(){
						...
						courseScoreEnter.save(myDialog,iframeObj,approvalStatus.TemporaryMemory.value); // 审核状态(1-暂存，2-已送审，3-已审核)			
						return false;	
					},focus:true},
					...
					{name:'送 审',callback:function(){
						var iframeObj = this.iframe.contentWindow;
						courseScoreEnter.review(myDialog,iframeObj,approvalStatus.Submitted.value); // 审核状态(1-暂存，2-已送审，3-已审核)
						return false;
					}}]
				});
		},	
		...
		popView: function(obj){				
			popup.data("obj",obj);//传值过去
			var myDialog= popup.open('./teacherservice/score/html/courseScoreEnterView.html',
				{
