			...
			if(queryParams.courseId==dataConstant.EMPTY){
				...
				return false;
					...
					if (data.code == config.RSP_SUCCESS) {
						...
						var mydialog = popup.open('./teacherservice/score/html/tacheScoreSet.html', // 这里是页面的路径地址
				                {
				                    id : 'set',// 唯一标识
				                    title : '环节成绩分制设置',// 这是标题
				                    ...
				                    ok : function(obj) {
			             				...
			             				reqDataCopy.academicYear = queryParams.academicYear;
			             	            reqDataCopy.semesterCode = queryParams.semesterCode;
					                        ...
					                        ajaxData.setContentType("application/json;charset=UTF-8");
					                     	ajaxData.request(url.SAVE_TACHE_SCORE_SET,JSON.stringify(reqre),function(data){
					               		    	if(data.code==config.RSP_SUCCESS)
					               		    	{
					               		    		...
					               		    		// 新增时不需要删除原始成绩  tacheScoreList.deleteOriginalScore();
					               		    		// 刷新列表						
						                        ...
						                        ajaxData.setContentType("application/json;charset=UTF-8");
						                     	ajaxData.request(url.SAVE_TACHE_SCORE_SET,JSON.stringify(reqre),function(data){
						               		    	if(data.code==config.RSP_SUCCESS)
						               		    	{
						               		    		...
						               		    		tacheScoreList.deleteOriginalScore();	
						               				}
						               			});
				            				});
				            			}
