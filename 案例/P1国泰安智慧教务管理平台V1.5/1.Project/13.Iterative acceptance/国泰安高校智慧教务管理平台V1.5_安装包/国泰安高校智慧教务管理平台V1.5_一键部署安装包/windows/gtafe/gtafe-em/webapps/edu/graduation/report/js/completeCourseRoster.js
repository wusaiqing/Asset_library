define(function(require, exports, module) {
	...
	var completeCourseRoster = {
		...
		init : function() {
			...
			ajaxData.request(
				...
				function(data){
					if(data && data.graduateYear){
						...
						completeCourseRoster.pagedList();
						completeCourseRoster.loadAcademicYearAndRelation1();
					}
				}
			);
			...
			$('#query').click(function() {
				...
				completeCourseRoster.pagination.setParam(data);
			});
		...
		pagedList:function(){
			...
			data["noCheckDepartment"]=false;
			//初始化列表数据
			completeCourseRoster.pagination = new pagination({
				id: "pagination", 
		...
		loadAcademicYearAndRelation1 : function(){
			...
			$("#grade").change(
					function() {
						...
						completeCourseRoster.majorChange("")
					});
			// 院系联动专业
			$("#departmentId").change(
				function() {
			...
			$("#majorId").change(function(){
				completeCourseRoster.majorChange($(this).val())
			});
		},
	...
	module.exports = completeCourseRoster; // 根据文件名称一致
	window.graduatingRoster = completeCourseRoster; // 根据文件名称一致
});