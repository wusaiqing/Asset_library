...
define(function(require, exports, module) {
	...
	var courselist = {
		...
		init : function() {
			...
			// 绑定开课单位下拉框
			simpleSelect.loadAuthStartClass("departmentId", null, "", constant.SELECT_ALL, "");
		...
		bindEvent:function(){
			...
			$("#btnLoadMore").click(function(){
				...
				param.courseId = me.courseList[me.courseNo].courseId;
				me.loadList(param);
			});
		...
		loadList:function(param){
			var me = this;
			ajaxData.request(urlCoursePlan.ARRANGE_SCHEDULE_GETCOURSESCHEDULELIST, param, function(data) {
				if (data.code === config.RSP_SUCCESS && data.data && data.data.length > 0) {
					var courseSchedule = data.data[0];

					if (me.courseNo) {
						me.courseNo = me.courseNo + 1;
						me.schedule.formatHtml(data.data[0],me.courseNo);
					} else {
						$("#courseCount").html(courseSchedule.courseCount);
							//时间设置 -- 一天多少节次
							var timesettings = me.timeSetting;
							//获取一周中从哪天开始上课
							me.schedule = new scheduleUtil("divBody",{
								weekNum: me.weekNum,
								fromhtml:"course", // class,teacher,teachRoom,course
								section:{
						...
						me.courseNo = 1;
						me.courseCount = courseSchedule.courseCount;
						me.courseList = courseSchedule.courseList;
					}
					
					if (me.courseCount > me.courseNo) {
						$("#btnLoadMore").show();
					} else {
						$("#btnLoadMore").hide();
					}
	...
	module.exports = courselist; // 根文件夹名称一致
});